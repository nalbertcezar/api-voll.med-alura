...
public class PacienteController {
	...
	public ResponseEntity<Page<DadosListagemPaciente>> listar(@PageableDefault(size = 10, sort = {"nome"}) Pageable page) {
		var produto =  repository.findAllByAtivoTrue(page).map(DadosListagemPaciente::new);
		return ResponseEntity.ok(produto);
	}
	
	@GetMapping("/{id}")
	public ResponseEntity listarDadosCompletos(@PathVariable Long id) {
	...
	@Transactional
	public void deletarPermanente(@PathVariable Long id) {
